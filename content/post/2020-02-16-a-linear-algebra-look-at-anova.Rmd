...
From any design matrix we can form an orthogonal projection matrix by $P_x=X(X^T X)^{-1} X^T$.  If we form $\hat{y}= P_x y$ we can think of the projection matrix as projecting our $y$ vector onto the column space spanned by the design matrix.
...
=\mathbf{y}(I-P_x)^T(I-P_x) \mathbf{y}\\
=\mathbf{y}(I-P_x) \mathbf{y}$$
...
=\mathbf{y}(I-Px_1)^T(I-Px_1) \mathbf{y} \\
=\mathbf{y}(I-Px_1) \mathbf{y}$$


Now, we note we can decompose the Total Sums of Squares by:

$$ \mathbf{y}(I-Px_1) \mathbf{y} = \mathbf{y}(I-Px_2+Px_2-Px_1) \mathbf{y} \\
= \mathbf{y}(I-Px_2) \mathbf{y} + \mathbf{y}(Px_2-Px_1) \mathbf{y}$$


Now we can think about this a bit.  The second term $\mathbf{y}(Px_2-Px_1) \mathbf{y}$ is describing the amount of Sums of Squares decreases when projected onto the column space of the full design matrix minus the amount of Sums of Squares decreases when projected onto intercept only column space.  If we look at the projection matrix for the full model it looks like:
...
$$ \mathbf{y}(I-Px_1) \mathbf{y} = \mathbf{y}(I-Px_3+Px_3-Px_2+Px_2-Px_1) \mathbf{y} \\
= \mathbf{y}(I-Px_3) \mathbf{y} + \mathbf{y}(Px_3-Px_2) \mathbf{y}+\mathbf{y}(Px_2-Px_1) \mathbf{y}$$
